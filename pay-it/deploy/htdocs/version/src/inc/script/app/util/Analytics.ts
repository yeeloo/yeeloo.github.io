...
//	console.log("trackPage::trackPage(" + category + ',' + action + ',' + label + ',' + value + ")");
	...
	else if (typeof _gaq !== 'undefined')
	{
		if (!isNaN(value) && label)
		{
			_gaq.push(<any[]>['_trackEvent', category, action, label, value]);
		}
		else if (label)
		{
			_gaq.push(<any[]>['_trackEvent', category, action, label]);
		}
		else
		{
			_gaq.push(<any[]>['_trackEvent', category, action]);
		}
	}
}
