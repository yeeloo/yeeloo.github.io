    ...
    var ArrayUtils = (function () {
        ...
        ArrayUtils.sortOn = function (array, fields, options) {
            ...
            if (options.length !== fields.length)
                options = [];
            if ((options[0] & ArrayUtils.UNIQUESORT) && (fields.some(function (field, i) { return dup_fn(field, options[i]); })))
                return 0;
         ...
         * @returns {Array}
         */
        ArrayUtils.concatAll = function (array) {
            var results = [];
            array.forEach(function (subArray) { return subArray.forEach(function (item) { return results.push(item); }); });
            return results;
        };
        /**
